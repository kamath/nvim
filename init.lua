require 'custom.anirudh.settings'
...
-- Add a keybinding for copy and paste
...
-- Function to get GitHub URL for current file
function _G.get_github_url()
  local remote_url = vim.fn.system('git config --get remote.origin.url'):gsub('\n', '')
  local repo_path = remote_url:match 'github%.com[:/](.+)%.git$'
  ...
  local is_visual = visual_marks[1][2] ~= 0 and visual_marks[2][2] ~= 0

  if is_visual then
    -- Visual mode
    line_start = visual_marks[1][2] -- Start line of visual selection
    line_end = visual_marks[2][2] -- End line of visual selection
  else
    -- Normal mode
  ...
  local url = string.format('https://github.com/%s/blob/%s/%s#L%d-L%d', repo_path, commit_sha, file_path, line_start, line_end)

  vim.fn.setreg('+', url)
  print('GitHub URL copied to clipboard: ' .. url)
end

-- Keymap to get GitHub URL
vim.api.nvim_set_keymap('n', '<Leader>g', ':lua _G.get_github_url()<CR>', { noremap = true, silent = true })
vim.api.nvim_set_keymap('v', '<Leader>g', ':lua _G.get_github_url()<CR>', { noremap = true, silent = true })
